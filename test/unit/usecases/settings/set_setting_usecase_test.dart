...
void main() {
  ...
  group('SetSettingUseCase', () {
    test('должен сохранить настройку с ключом и значением', () async {
      ...
      when(
        mockRepository.setValue(testKey, testValue, encrypted: false),
      ).thenAnswer((_) async {});
    ...
    test('должен сохранить настройку с encrypted=true', () async {
      ...
      when(
        mockRepository.setValue(testKey, testValue, encrypted: true),
      ).thenAnswer((_) async {});
    ...
    test(
      'должен вызвать repository.setValue с правильными параметрами',
      () async {
        // Arrange
    ...
    test('должен работать с разными значениями', () async {
      // Arrange
      when(
        mockRepository.setValue('key1', 'value1', encrypted: false),
      ).thenAnswer((_) async {});
      when(
        mockRepository.setValue('key2', 'value2', encrypted: true),
      ).thenAnswer((_) async {});
