...
class PasswordHistoryRepositoryImpl implements PasswordHistoryRepository {
  ...
  Future<Either<PasswordHistoryFailure, int>> saveHistoryEntry({
    ...
    try {
      ...
      return Left(PasswordHistoryFailure(message: 'Ошибка сохранения истории: $e'));
    }
  }

  @override
  Future<Either<PasswordHistoryFailure, List<PasswordHistoryEntry>>> getHistoryForEntry(
    int entryId,
  ) async {
    try {
      final db = await dbHelper.database;
      ...
      return Left(PasswordHistoryFailure(message: 'Ошибка получения истории: $e'));
    }
  }

  @override
  Future<Either<PasswordHistoryFailure, PasswordHistoryEntry?>> getLastHistoryEntry(
    int entryId,
  ) async {
    try {
      final db = await dbHelper.database;
      ...
      return Right(PasswordHistoryEntry(
        id: map['id'] as int?,
        entryId: map['entry_id'] as int,
        service: map['service'] as String,
        encryptedPassword: map['encrypted_password'] as String,
      ...
      return Left(PasswordHistoryFailure(message: 'Ошибка получения последней записи: $e'));
    }
  }

  @override
  Future<Either<PasswordHistoryFailure, int>> getHistoryCount(int entryId) async {
    try {
      final db = await dbHelper.database;
      ...
      return Left(PasswordHistoryFailure(message: 'Ошибка получения количества записей: $e'));
    }
  }

  @override
  Future<Either<PasswordHistoryFailure, bool>> deleteHistoryForEntry(int entryId) async {
    try {
      final db = await dbHelper.database;
      ...
      return Left(PasswordHistoryFailure(message: 'Ошибка удаления истории: $e'));
    }
  }
  ...
  Future<Either<PasswordHistoryFailure, int>> pruneOldHistory({
    ...
    try {
      ...
      return Left(PasswordHistoryFailure(message: 'Ошибка очистки старой истории: $e'));
    }
  }

  @override
  Future<Either<PasswordHistoryFailure, Map<String, dynamic>>> getHistoryStats() async {
    try {
      final db = await dbHelper.database;
      ...
      final entriesWithHistory = entriesWithHistoryResult.first['count'] as int? ?? 0;
      ...
      return Left(PasswordHistoryFailure(message: 'Ошибка получения статистики: $e'));
    }
  }
}
