...
Uint8List radToBytes(String str, int rad) {
  if (str.length % 2 != 0) {
    throw const FormatException(
      'Длина строки должна быть кратна 2, так как каждый байт кодируется 2 символами',
    );
  }
  ...
  for (int i = 0; i < str.length; i += 2) {
    ...
    if (byte < 0 || byte > 255) {
      throw FormatException(
        'Некорректное значение байта: $byte из "$byteStr" при rad=$rad',
      );
    }
    bytes.add(byte);
  }
...
Uint8List encrypt(Uint8List data, Uint8List key, {int byteLength = 0x10}) {
  ...
  final Uint8List newData = Uint8List(data.length);
...
Uint8List decrypt(Uint8List data, Uint8List key, {int byteLength = 0x10}) {
  ...
  final Uint8List hashData = Uint8List(32);
  for (int i = 0; i < 32; i++) {
    hashData[i] = data[data.length - 32 + i];
  }
  final Uint8List newData = Uint8List(data.length - 16 - 16 - 64);
