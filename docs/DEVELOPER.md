...
| Последний запуск тестов | 182 passed, 0 failures |
...
| v3 | Argon2id | заготовка, не используется | перспективная версия без полноценной настройки memory cost/parallelism |

Текущая версия `EncryptionVersion.currentVersion` равна `2`.
В коде `EncryptionParams.v3()` содержит поле `iterations: 3`, но это только
заготовка структуры параметров, а не полноценная реализация Argon2id. Для
реального Argon2id потребуется явно задать memory cost, parallelism и формат
хранения параметров.
...
Пункт "Очистить логи" вызывает `ClearLogsUseCase`, который делегирует полную
очистку в `SecurityLogRepository.clearAll()`.
...
Ранее сгенерированные Mockito mocks для `SecurityLogRepository` устарели после
изменения интерфейса: в методах появились named-параметры `profileId`.
Проблема устранена регенерацией mocks.
...
После регенерации выполнена проверка:
...
7. Версии проекта требуют выравнивания: `pubspec.yaml`, `AppConstants` и
